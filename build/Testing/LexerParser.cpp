...
void LexerParser::_collectArguments(std::string& strArgs, int argc, const char** argv) noexcept
{
...
void LexerParser::_extractKey(const char* reader)
{
    ...
    else if (*reader++ == '-')
    {
        ...
        {
            if (_keyExists(keys, *reader)) throw std::runtime_error(std::string("Key -") + *reader + " is not unique");
